...
class Bar: uvm_object
{
  ...
  void check_setting(int XLEN,satp_mode_t SATP_MODE)
  {
    ...
    if (support_128b && XLEN != 128)
      {
	uvm_fatal(get_full_name(), "XLEN should be set to 128 based on riscv_instr_pkg.supported_isa setting");
      }
    if (!support_128b && support_64b && XLEN != 64)
      {
	uvm_fatal(get_full_name(), "XLEN should be set to 64 based on riscv_instr_pkg.supported_isa setting");
      }
   if (!(support_128b || support_64b) && XLEN != 32)
      {
	uvm_fatal(get_full_name(), "XLEN should be set to 32 based on riscv_instr_pkg.supported_isa setting");
      }
	...
	uvm_fatal(get_full_name(), format("SATP mod %0s is not supported for RV32G ISA", SATP_MODE));
      }
  }
...
void main()
{
    auto test = Bar.type_id.create("test");
    test.check_setting(64,satp_mode_t.SV64);
